...
public class Bider extends Agent{
	...
	public void bider0_strategy(int bid) {//Anevazei ana 1
		
		if(bid>budget)
			bid=0;
		else if(bid==budget)
			bid=budget;
		else if(bid+1<budget)
			bid+=1;
		else
			bid=0;
		System.out.println("bider0 sending:"+bid);
		send_msg(Integer.toString(bid));
		if(bid > 0)prev_bid = bid;
	}
	public void bider1_strategy(int bid) {//Anevazei ana 1
		
		if(bid>budget)
			bid=0;
		else if(bid==budget)
			bid=budget;
		else if(bid+100<budget)
			bid+=100;
		else
			bid=0;
	...
	public void bider2_strategy(int bid) {//Anevazei ana 1
